...
<script type="text/javascript">
	var meetNoticeselfUrl = "${ctx }/meetSmsReadySendAction/getBatchSms.action?batchId=<%=batchId%>&smsReadySendId=<%=smsReadySendId%>",
	meetNoticeself_export_url = "${ctx }/meetSmsReadySendAction/exportResult.action?batchId=<%=batchId%>&smsReadySendId=<%=smsReadySendId%>";
	meetNoticecurrent_url = meetNoticeselfUrl;
	meetNoticeexport_sms_url = meetNoticeself_export_url;
	meetNoticerightMenuBar.showReadySendResultMenu();
	var resultGridPro = {
		url:"${ctx }/meetSmsReadySendAction/listBatchSendResult.action?batchId=<%=batchId%>&sendResult=<%=sendResult%>&receiverName="+encodeURI("<%=receiverName%>")+"&receiveMoble="+encodeURI("<%=receiveMoble%>")+"&failReason="+encodeURI("<%=failReason%>"),
		...
		buttons: [{
			...
			click: function(){
				...
				//window.location.href="${ctx }/meetSmsReadySendAction/getSmsDetails.action?selectedId=<%=smsReadySendId%>";
			}
		}],
		multiselect: true,
		height: 390
	...
	var readyResultGrid;
	$(function() {
		readyResultGrid = new TableGrid("meetNoticereadyResultGrid",resultGridPro);
	...
	<td valign="top">
		<div class="left_contents">
			...
			<div class="table_box1">
			    <table id="meetNoticereadyResultGrid"></table>
			</div>
		</div>
	</td>
  </tr>
