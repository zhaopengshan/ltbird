	 ...
	 * @returns {} 
	 */
	function confirmAjaxFunc(meetNoticesmsGrid,url,showMessage){
		var ids = meetNoticesmsGrid.getSelectedItemIds();
		if( ids.length > 0 ){
	 ...
	 * @returns {} 
	 */
	function collectSmsFunc(meetNoticesmsGrid,url){
		var ids = meetNoticesmsGrid.getSelectedItemIds();
		if( ids.length > 0 ){
        	$.ajax({
                url : url,
	 ...
	 * @returns {} 
	 */
	function meetNoticeeditSmsFunc(meetNoticesmsGrid,url){
		var rows = meetNoticesmsGrid.getSelectedItemIds();
		if( rows.length == 1 ){ 
	...
	ValidateForm.prototype={
		init: function(){
			//归一化
			if(typeof this.formId == "string") {if(this.formId.substr(0,1) !== "#") { this.formId = "#meetNotice"+this.formId;}
		...
		//ValidateForm.validate("#meetNoticeinputid","#meetNoticeoutId",[1,2],["required","validateString"]);
		validate: function(formTar,exportTar,param,funcName){
			...
			//归一化
			if(typeof formTarId == "string") {if(formTarId.substr(0,1) !== "#") { formTarId = "#meetNotice"+formTarId;}
			}else { formTarId = "#meetNotice"+ $(formTarId).attr("id");}
			//归一化
			if(typeof exportId == "string") {if(exportId.substr(0,1) !== "#") { exportId = "#meetNotice"+exportId;}
			}else { exportId = "#meetNotice"+ $(exportId).attr("id");}
		...
		validateForm: function(){
			var self = this,
				...
				if( !reResult ){
					$("#meetNotice"+this.outputTarId).removeClass("ui-helper-hidden");
					setTimeout((function(outputTarId){
						return function(){ 
							$("#meetNotice"+outputTarId).addClass("ui-helper-hidden"); 
						};
