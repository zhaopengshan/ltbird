...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    this.tabpanel_content.addClass('tabpanel_content').css("overflow-x","hidden");
  ...
  show : function(position, notExecuteMoveSee){
    ...
    //sunyadong add
    for(var i = 0; i < this.tabs.length; i++){
    	this.tabs[i].content.addClass("ui-helper-hidden");
    }  
    ...
    if(this.scrollFinish)
    {
      ...
      this.tabs[position].content.css('z-index', ++this.maxzindex).removeClass("ui-helper-hidden");
      if(this.tabs[position].tab.hasClass('active'))
      {
  	  ...
  	  {	
    	  /*************自己扩展，选中选项卡时，左边的菜单相应的选中*********************/
    	  // sid : 父菜单名_子菜单名_父菜单id_子菜单id
    	  /*var sid = this.tabs[position].id;//alert(sid);
    	  
    	  var nid = sid.split("_"); //substring(sid.indexOf("_"));

    	  if(nid[1] == "null")
    		  $("#currLocation").html("当前位置："+nid[0]);
    	  else 
    		  $("#currLocation").html("当前位置："+nid[0]+">"+nid[1]);
    	  ...
    	  // 去掉之前所选的子菜单样式
  	  	  ...
  	  	  // 给当前菜单加上选择样式
    	  $("#"+nid[3]).addClass('on');
    	  // 给当前父菜单加current样式并展开其下所有的子菜单
    	  $("#"+nid[2]).parent().addClass("current").find("div.pane").show("30");
    	  // 隐藏其它父菜单的子菜单列表
    	  $("#"+nid[2]).parent().siblings().find("div.pane").hide("30");
    	  
    	  
    	  //$("#"+nid[2]).parent().removeClass("current").find("div.pane").show("50");
    	  //$("#"+nid[2]).parent().siblings().removeClass("current").find("div.pane").hide("50");*/
  ...
  kill : function(position){
	...
	//sunyadong add check mark 20130906
	var tabTitle = this.getTitle(position);
	switch(tabTitle){
		case "联系人导入": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭导入联系人标签？不建议导入联系人时关闭此标签！") ){
					return;
				}
			}
		break;
		case "发送短信": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭发送短信标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
		case "发送会议通知": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭发送会议通知标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
		case "创建日程": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭创建日程标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
		case "创建投票": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭创建投票标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
		case "编辑短信答题": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭编辑短信答题标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
		case "抽奖任务": 
			if( typeof(arguments[1]) == "undefined" ){
				if( !confirm("确定关闭抽奖任务标签？不建议提交中关闭此标签！") ){
					return;
				}
			}
		break;
