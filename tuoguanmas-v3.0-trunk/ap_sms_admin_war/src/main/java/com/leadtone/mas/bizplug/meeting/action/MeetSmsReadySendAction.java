...
import com.leadtone.mas.bizplug.sms.bean.MbnSmsReadySendCreateByVO;
...
@Namespace(value = "/meetSmsReadySendAction")
public class MeetSmsReadySendAction extends BaseAction {
		...
		if(!StringUtil.isEmpty(batchId)){
			...
			try{
				...
				if(!StringUtil.isEmpty(failReason)){
					String temp=URLDecoder.decode(failReason,"UTF-8");
					if(!temp.equals("全部")){
						pageUtil.setFailReason(temp);
					}
				}
	...
	@Action(value = "getBatchSms",
			results = { @Result(name = SUCCESS, location = "/sms/meeting/jsp/readysend_result_details.jsp"),
	...
	public String getBatchSms(){
		if( !StringUtil.isEmpty(batchId)){
			try{
				...
				smsReadySendList = mbnSmsReadySendService.getByBatchId(Long.valueOf(batchId), u.getMerchantPin(), createById);
	...
	results = { @Result(name = SUCCESS, location = "/sms/meeting/jsp/readysend_content.jsp"),
				@Result(name = ERROR, location = "/error.jsp")})
	public String followPage(){
		if( !StringUtil.isEmpty(selectedId) && pageDirect != null){
			...
			try {
				...
				smsReadySendList = mbnSmsReadySendService.getByBatchId(mbnSmsReadySendVO.getBatchId(), u.getMerchantPin(), createById);
				UserVO uv=new UserVO();
				uv.setId(mbnSmsReadySendVO.getCreateBy());
	...
	results = { @Result(name = SUCCESS, location = "/sms/meeting/jsp/readysend_content.jsp"),
			@Result(name = "refresh", type="redirectAction", location = "../meetSmsHadSendAction/getSmsDetails.action", 
	...
	public String listSmsReadySend(){
		...
		try{
			...
			if( page != null ){
				@SuppressWarnings("unchecked")
				List<MbnSmsReadySendCreateByVO> datas = (List<MbnSmsReadySendCreateByVO>) page.getData();
				entityMap = new HashMap<String, Object>();
	            entityMap.put("total", page.getRecords());
	            if( datas == null ){
	            	datas = new ArrayList<MbnSmsReadySendCreateByVO>();
	            }
	            entityMap.put("rows", datas); 
	            entityMap.put("totalrecords", page.getTotal());
	            entityMap.put("currpage", page.getStart());
		...
		try{
			...
			if( page != null ){
	            ...
	            for (int j = 0; j < size; j++) {
	                ...
	                billsInArr[j][0] = smsStatus;	// 状态 -1取消发送,0未发送,1已提交网关,2成功,3失败 
	                billsInArr[j][1] = mbnSmsReadySend.getTitle();		// 短信标题
	                billsInArr[j][2] = mbnSmsReadySend.getContent();		//  短信内容 
	                billsInArr[j][3] = ""+mbnSmsReadySend.getContent().length();	//短信长度
	                billsInArr[j][4] = (null == mbnSmsReadySend.getReadySendTime()  ? "" : sdf.format(mbnSmsReadySend.getReadySendTime()));	// 发送时间
	...
	public String exportReply(){
		...
		try{ 
			...
			pageUtil.setBatchId(Long.parseLong(batchId));
			if(!StringUtil.isEmpty(searchBycontacts)){
				pageUtil.setCreateByName(searchBycontacts);
			}
			if(!StringUtil.isEmpty(searchAct)){
				Timestamp endDate = new Timestamp(System.currentTimeMillis());
				switch(Integer.valueOf(searchAct)){
