...
public class SmsSendAction extends BaseAction {
	...
	@Action(value="smsCheckLoginGet" ,results = { @Result(type = "json", params = {
			"root", "entityMap", "contentType", "text/html" }) })
	public String smsCheckLoginGet() {
		Users loginUser = userService.queryByUserId(portalUserExt.getId());
		String prov = loginUser.getProvince();
...
//		ydTunnel = BizUtils.getYdTunnelId(mbnMerchantTunnelRelationService, loginUser.getMerchantPin());
//		if( ydTunnel == null || ydTunnel <= 0 ){
...
//        }
		...
		smsText = "本次登录短信验证码是【"+checkNumber+"】";
		Contacts rec = new Contacts(tempUserExt.getSmsMobile(), tempUserExt.getSmsMobile());
		userSet.add(rec);
		sendEasyMain( userSet, loginUser );
 		return SUCCESS;
	}
	/**
	 * 
	 * @return
	 */
	@Action(value="smsCheckSettingGet" ,results = { @Result(type = "json", params = {
			"root", "entityMap", "contentType", "text/html" }) })
	public String smsCheckSettingGet() {
		...
		if(tempUserExt.getSmsMobile()!=null&&!tempUserExt.getSmsMobile().trim().equals("")){
			Set<Contacts> userSet = new HashSet<Contacts>();
			if(!receiver.trim().equals(tempUserExt.getSmsMobile())){
				smsText = "短信验证码接收手机号即将发生变动，如非本人操作请联系管理员!";
				Contacts rec = new Contacts(tempUserExt.getSmsMobile(), tempUserExt.getSmsMobile());
				userSet.add(rec);
				sendEasyMain( userSet, loginUser );
			}
			smsText = "短信验证码接收手机号即将设置，本次验证码是【"+checkNumber+"】";
			userSet = new HashSet<Contacts>();
			Contacts rec = new Contacts(receiver.trim(), receiver.trim());
			userSet.add(rec);
			sendEasyMain( userSet, loginUser );
		}else{
			smsText = "短信验证码接收手机号即将设置，本次验证码是【"+checkNumber+"】";
	...
	private void sendEasyMain(Set<Contacts> userSet, Users loginUser ){
		List<SmsMbnTunnelVO>  tunnelList = getTunnelList(loginUser.getMerchantPin());
		...
		for (Contacts info : userSet) {
			...
			if( hcode != null){
				...
				String fourthNumber = StringUtil.getShortPrefixFourth(info.getMobile());
				switch(fourthNumber){
					case"0": container.addContacts(info, "dx"); break;
					case"5": container.addContacts(info, "yd"); break;
					case"9": container.addContacts(info, "lt"); break;
					default: ;
				}
			}
		}
		...
		MbnSmsOperationClass mbnSmsOperationClass 
			= mbnSmsOperationClassService.findByCoding(ApSmsConstants.OPERATION_CODING_YZ);
		int priority = mbnSmsOperationClass.getPriorityLevel().intValue();
		
		String[] result = rebuildContainerForHostMas(loginUser, container, tunnelList);
//        if( Boolean.parseBoolean(result[0])){
			...
			MbnSmsReadySendContainer smsContainer = buildContainerForHostMas(
					loginUser, container, batchId, readySendTime, priority,
					colsList, taskNumber);
			mbnSmsReadySendService.batchSave(smsContainer);
			// 增加发送结果
			entityMap = new HashMap<String, Object>();
			entityMap.put("resultcode", "success");
			entityMap.put("message", "短信验证码发送成功！");
//			this.getRequest().setAttribute("entityMap", entityMap);
	}
...
//        }
        ...
        if(WebUtils.isHostMas()) {//to
            ...
            MbnSmsReadySendContainer smsContainer = buildContainer(loginUser, container, batchId, readySendTime, priority, colsList, taskNumber);
            mbnSmsReadySendService.batchSave(smsContainer);
            // 增加发送结果
            entityMap = new HashMap<String, Object>();
            entityMap.put("resultcode", "success" );
...
//		this.getRequest().setAttribute("tunnelList", tunnelList);
	...
	@Action(value = "writesms", results = {
			@Result(name = SUCCESS, location = "/sms/smssend/jsp/sms_write.jsp"),
			@Result(name = "dynamic", location = "/sms/smssend/jsp/dynamic_sms_write.jsp"),
			@Result(name = INPUT, location = "/login.jsp") })
	 ...
	 * @return
	 */
	private String[] rebuildContainerForHostMas(Users loginUser, ContactsContainer container, List<SmsMbnTunnelVO>  tunnelList){
		ydTunnel=0L;
		ydOtherTunnel=0L;
		ltTunnel=0L;
		dxTunnel=0L;
...
//			return result;
//		}
//		sunyadong add 20141020
		try{
			for(SmsMbnTunnelVO svo:tunnelList){
				provinceCode = svo.getProvince();
				if( (svo.getTunnelRange()&1) == 1){
					this.ydTunnel = svo.getId();
					if( container.getYdList().size() > 0){
						for( Contacts contacts: container.getYdList()){
							...
							if( code!=null && provinceCode.equalsIgnoreCase(code.getProvinceCoding()) ){
								container.addYdSelfList(contacts);
							}
	//						if( code == null || !provinceCode.equalsIgnoreCase(code.getProvinceCoding())){
	...
	//						}
				...
				if( (svo.getTunnelRange()&2) == 2 ){
					this.ydOtherTunnel = svo.getId();
					if( container.getYdList().size() > 0){
						for( Contacts contacts: container.getYdList()){
							MbnSevenHCode code = mbnSevenHCodeService.queryByBobilePrefix(StringUtil.getLongPrefix(contacts.getMobile()));
							if( code == null || !provinceCode.equalsIgnoreCase(code.getProvinceCoding())){
								container.addYdOtherList(contacts);
	//							if(svo.getClassify()==ApSmsConstants.TUNNEL_CLASSIFY_YD 
	//									||svo.getClassify()==ApSmsConstants.TUNNEL_CLASSIFY_GLOBAL){
	//							//	container.addYdContacts(contacts);
	//							} else {
	...
	//							}
				...
				if( (svo.getTunnelRange()&4) == 4 ){
					this.ltTunnel = svo.getId();
				}
				if( (svo.getTunnelRange()&8) == 8 ){
					this.dxTunnel = svo.getId();
				}
				// 将猫池发送从YD列表中删除
		...
		if( ltTunnel > 0){
			...
			if( svo == null){
				...
				if( mmc.getRemainNumber() < container.getLtList().size()){
					finalResult = false;
					ltTip = "联通帐户余额" + mmc.getRemainNumber()+"条实际"+container.getLtList().size()+"条";
				}
				else{
					// 判断是否省分通道
	...
	private MbnSmsReadySendContainer buildContainerForHostMas(Users loginUser, ContactsContainer container, 
		...
		// 构建移动通道短信列表
		if( container.getYdSelfList() != null &&  container.getYdSelfList().size() > 0){
			List<MbnSmsReadySend> msrsList = buildSmsList(loginUser, 
					container.getYdSelfList(), batchId, readySendTime, priority, ydTunnel, colsList, taskNumber);
			smsContainer.addSmsMap(ydTunnel, msrsList);
		}
		//ydOtherTunnel
		if( container.getYdOtherList() != null &&  container.getYdOtherList().size() > 0){
			List<MbnSmsReadySend> msrsList = buildSmsList(loginUser, 
					container.getYdOtherList(), batchId, readySendTime, priority, ydOtherTunnel, colsList, taskNumber);
			smsContainer.addSmsMap(ydOtherTunnel, msrsList);
		}
		// 构建联通通道短信列表
...
//                		|| svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_QXT_NEW
//                		|| svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_LT||svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_SELF_LT
//                		|| svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_DX||svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_SELF_DX
//                		||svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_YDYW) {
//                	msrs.setContent(getSmsText() + (getReplyText()==null?"":getReplyText()) + (getEntSign()==null?"":getEntSign()));
//                } else {
//                    msrs.setContent(getSmsText() + (getReplyText()==null?"":getReplyText()));
//                }
                if( svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_SELF_YD || svo.getClassify() == ApSmsConstants.TUNNEL_CLASSIFY_YD ) {
...
//                }
            ...
            }else if( tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_QXT || tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_QXT_NEW ){
            	// 企信通
//            	msrs.setSmsAccessNumber(String.valueOf(loginUser.getZxtId()));
            	msrs.setSmsAccessNumber(String.valueOf(loginUser.getZxtUserId()));
            }else if( tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_DX || tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_SELF_DX
            		|| tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_LT|| tunnelType == ApSmsConstants.TUNNEL_CLASSIFY_SELF_LT){
