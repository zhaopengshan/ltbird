...
public class AddrAction extends BaseAction {
	...
	public String findAddrByParentId() {
		...
		if( !Boolean.valueOf( WebUtils.getPropertyByName(ApSmsConstants.PRIVATEGROUPOPEN))
				|| loginUser.getUserType() == ApSmsConstants.USER_TYPE_ENTERPRISE_ADMIN){
			createBy = null;
		}
		Long id = -100L;
		if (s != null) {
		...
		this.contactsList = contactsService.getContactsByParentId(loginUser.getMerchantPin(), id , createBy, page<1?1:page, 20);
		return SUCCESS;
	}
