...
public class MbnSmsHadSendAction extends BaseAction {
	...
	private static Log LOG = LogFactory.getLog(MbnSmsHadSendAction.class);
	/**
	 * 
	 */
	...
	@Action(value = "getBatchSms",
			results = { @Result(name = SUCCESS, location = "/sms/smssend/jsp/hadsend_result_details.jsp"),
	...
	public String getBatchSms(){
		if( !StringUtil.isEmpty(batchId)){
			try{
				...
				smsHadSendList = mbnSmsHadSendService.getByBatchId(Long.valueOf(batchId), u.getMerchantPin(), createBy);
	...
	results = { @Result(name = SUCCESS, location = "/sms/smssend/jsp/hadsend_content.jsp"),
				@Result(name = ERROR, location = "/error.jsp")})
	public String followPage(){
		if( !StringUtil.isEmpty(selectedId) && pageDirect != null ){
			...
			try {
				...
				smsHadSendList =  mbnSmsHadSendService.getByBatchId(mbnSmsHadSendVO.getBatchId(), u.getMerchantPin(), createBy);
				return SUCCESS;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public String export(){
		...
		try{
			...
			if( page != null ){
                ...
                billsInArr[j][5] = (null == mbnSmsHadSendVO.getReadySendTime()  ? "" : sdf.format(mbnSmsHadSendVO.getReadySendTime()));	// 发送时间
