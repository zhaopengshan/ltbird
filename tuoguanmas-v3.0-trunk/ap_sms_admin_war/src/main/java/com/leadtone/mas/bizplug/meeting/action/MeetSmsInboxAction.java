...
@Namespace(value = "/meetSmsInboxAction")
public class MeetSmsInboxAction extends BaseAction {
	...
	results = { @Result(name = SUCCESS, location = "/sms/meeting/jsp/inbox_content.jsp"),
				@Result(name = ERROR, location = "/error.jsp")})
		...
		try{
			...
			if( page != null ){
	            ...
	            for (int j = 0; j < size; j++) {
	            	...
	            	String senderName = mbnSmsInbox.getSenderMobile();
	            	String sName = mbnSmsInbox.getSenderName();
					if( sName != null && sName.trim() != "" ){
						senderName += "<"+ sName + ">";
					}
	                billsInArr[j][0] = smsStatus;	
	                billsInArr[j][1] = senderName;		// 发件人
	                billsInArr[j][2] = mbnSmsInbox.getContent();		// 短信内容 
	                billsInArr[j][3] = ""+mbnSmsInbox.getContent().length();	// 短信长度
	                billsInArr[j][4] = (null == mbnSmsInbox.getReceiveTime() ? "" : sdf.format(mbnSmsInbox.getReceiveTime()));	// 接收时间
	            }
	            String[] cols = {"状态", "发件人", "短信内容", "短信长度", "接收时间"};
