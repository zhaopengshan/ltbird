	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					//add sunyadong
					//sunnav = "<td dir='"+dir+"'><span id='sp_nav_"+$.jgrid.jqID(pgid)+"'>"+ts.p.navtext+ "</span></td>";
					sppg = ts.p.toppager ? '#sp_nav'+tspg+",#sp_nav"+tspg_t : '#sp_nav'+tspg;
					$(sppg).html($.fmatter ? $.jgrid.format(ts.p.navtext || "", ts.p.page, ts.p.lastpage):ts.p.lastpage);
					//add end
					//sunyadong remove tow line
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pginput===true) { 
				...
				pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.showtext || "","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span><input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>")+"</td>";
				sunnav = "<td dir='"+dir+"'><span id='sp_nav_"+$.jgrid.jqID(pgid)+"'></span></td>";
			}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir=="rtl") { po.reverse(); }
				//sunyadong edit 2012.12.20
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'>首页</td>";
				pgl += sep;
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'>上一页</td>";
				pgl += sep;
				pgl += sunnav;
				pgl += sep;
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'>下一页</td>";
				pgl += sep;
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'>尾页</td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				//edit end
		...
		*
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: "200px"}).appendTo(eg).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');
		...
		*/
		...
		if(ts.p.pager){
			...
			//$(ts.p.pager).css({width: grid.width+"px"}).appendTo(eg).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');
			$(ts.p.pager).css({width: "200px"}).appendTo(grid.ubDiv).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');
			if(hg) {$(ts.p.pager).hide();}
			setPager(ts.p.pager,'');
		}
		//end
		if(ts.p.toppager) {
			...
			//end
			$(grid.topDiv).appendTo(grid.uDiv).addClass('ui-state-default ui-jqgrid-toppager').width("200px");
