...
<!-- 
	var gridPro = {
		url:"../../mbnSmsReadySendAction/listSmsReadySend.action?searchAct=<%=searchAct%>&dateFrom=<%=dateFrom%>&dateTo=<%=dateTo%>&searchBycontacts="+encodeURI("<%=searchBycontacts%>")+"&searchBySmsTitle="+encodeURI("<%=searchBySmsTitle%>"),
		colNames:['id','状态','短信标题','短信内容', '短信长度','发送时间','回复','批次号'],
		colModel:[
			{name:'id',index:'id', width:0,sortable: false, align: 'center',hidden: true,sorttype:'number',formatter:function(cellvalue, options, rowObject){
			...
			{name:'sendResult',index:'sendResult', width:70,sortable: false, align: 'center',formatter:function(cellvalue, options, rowObject){
					var iconSms = '<img title="未发送" src="../../images/u16220_normal.png" width="15" height="13">';
					switch(parseInt(cellvalue,10)){
						case -1: iconSms = '<img title="已取消" src="../../images/sms_cancel_normal.png" width="15" height="16">'; break;
						case 0: iconSms = '<img title="未发送" src="../../images/u16220_normal.png" width="13" height="16">'; break;
						case 1: iconSms = '<img title="发送中" src="../../images/u1223_normal.png" width="15" height="13">'; break;
					}
					return iconSms;
				}},
			...
			{name:'content',index:'content', width:250,sortable: false,formatter:function(cellvalue, options, rowObject){
					if($.trim(cellvalue) == "" ){
						...
						return '<a href="../../mbnSmsReadySendAction/getSmsDetails.action?selectedId='+ rowObject.id +'" class="grid-sms">'+cellvalue+'</a>';
					}
				}},
			...
			{name:'readySendTime',index:'readySendTime', width:130,sortable: false, align: 'center',sorttype:'date', datefmt:'Y-m-d'},		
			{name:'replyCount',index:'replyCount', width:60,sortable: false, align: 'center',formatter:function(cellvalue, options, rowObject){
				//此方法用的是已发箱里的url,因为相关的操作与已发箱的回复信息列表一样
					return "<a href='${ctx }/smssend/jsp/readysend_replyInfo.jsp?batchId="+rowObject.batchId+ "&smsReadySendId="+rowObject.id+"&smsRe=0'>"+cellvalue+"条</a>";
				}},
			{name:'batchId',index:'batchId',hidden: true, width:110,sortable: false, align: 'center'}
		],
		pager: '#smsPage',
		buttons: [{
			text: "取消发送",
			...
			classes: "tubh",
			click: function(){
				var url = '../../mbnSmsReadySendAction/collectByIds.action';
                collectSmsFunc(url);
			}
		}]
	};
