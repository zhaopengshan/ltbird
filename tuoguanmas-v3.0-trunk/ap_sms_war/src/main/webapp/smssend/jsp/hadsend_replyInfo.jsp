...
<!-- 
	var selfUrl = "${ctx }/smssend/jsp/hadsend_replyInfo.jsp?batchId=<%=batchId%>&smsHadSendId=<%=smsHadSendId%>";
	<%if(smsRe!=null){%>
		window.parent.current_url = selfUrl+"&smsRe=<%=smsRe%>";
	<%}else{%>
		window.parent.current_url = selfUrl;
	<%}%>
	window.parent.rightMenuBar.showReplyHadSendMenu();
	var resultGridPro = {
		...
		colNames:['id','tos','回复人','回复内容', '回复时间'],
		colModel:[
			{name:'id',index:'id', width:0,sortable: false, align: 'center',hidden: true},
			{name:'senderMobile',index:'senderMobile', width:0,sortable: false, align: 'center',hidden: true},
			...
			{name:'content',index:'content', width:100,sortable: false, align: 'center'}, 
		...
		buttons: [{
			...
			click: function(){
				...
				<%}else{%>
					window.parent.current_url = "${ctx }/mbnSmsHadSendAction/getSmsDetails.action?selectedId=<%=smsHadSendId%>";
				<%}%>
			...
			click: function(){
				...
				if( rows.length == 1 ){ 
					window.parent.parent.addTabs('itemx','itemxs'+rows,'短信互动','快速回复',url+'?selectedId='+rows);
	...
	var smsGrid;
	$(function() {
		smsGrid = new LeadToneGrid("hadsentReply",resultGridPro);
		smsGrid.init();
	});
	 -->
</script>
