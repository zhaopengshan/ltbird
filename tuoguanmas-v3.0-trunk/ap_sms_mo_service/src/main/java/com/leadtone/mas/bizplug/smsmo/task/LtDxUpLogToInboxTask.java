...
public class LtDxUpLogToInboxTask implements Callable<Integer> {
	
	private Logger logger = Logger.getLogger(LtDxUpLogToInboxTask.class);
	private SmsMoHandleService service;
	
	public LtDxUpLogToInboxTask(SmsMoHandleService serviceTemp){
		service = serviceTemp;
	}
	...
	public Integer call(){
		try{
			List<SmsMoLogBean> listSmsMoLogBean = service.getSmsMoLogs(0, "SMGP");
...
//			}
			for (Iterator<SmsMoLogBean> it = listSmsMoLogBean.iterator(); it.hasNext();) {
				...
				logger.info("smgp uplog to inbox "+smsMoLogBean.toString());
				try {
					...
					logger.error("LTDX up log to inbox 短信处理异常smsMoLogBean "+smsMoLogBean.toString(), e);
				}
			}
		}catch(Exception e){
			logger.error("LTDX up log to inbox 短信处理异常",e);
		}
		return 1;
	}
