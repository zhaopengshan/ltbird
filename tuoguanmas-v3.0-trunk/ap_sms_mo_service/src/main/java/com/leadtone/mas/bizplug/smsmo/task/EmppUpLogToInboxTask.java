...
public class EmppUpLogToInboxTask implements Callable<Integer> {
	
	private Logger logger = Logger.getLogger(EmppUpLogToInboxTask.class);
	private SmsMoHandleService service;
	
	public EmppUpLogToInboxTask(SmsMoHandleService serviceTemp){
		service = serviceTemp;
	}
	...
	public Integer call(){
		try{
			List<SmsMoLogBean> listSmsMoLogBean = service.getSmsMoLogs(0, "EMPP");
...
//			}
			for (Iterator<SmsMoLogBean> it = listSmsMoLogBean.iterator(); it.hasNext();) {
				...
				logger.info("empp uplog to inbox "+smsMoLogBean.toString());
				try {
					...
					logger.error("YD empp up log to inbox 短信处理异常smsMoLogBean "+smsMoLogBean.toString(), e);
				}
			}
		}catch(Exception e){
			logger.error("YD empp up log to inbox 短信处理异常",e);
		}
		return 1;
	}
