...
public class LtDxMoToUpLogTask implements Callable<Integer> {
	
	private Logger logger = Logger.getLogger(LtDxMoToUpLogTask.class);
	
	private GwSmsMoLogDao gwSmsMoLogDao;
	private SmsMoLogDao smsMoLogDao;
	
	public LtDxMoToUpLogTask(GwSmsMoLogDao gwSmsMoLogDaoTemp, SmsMoLogDao smsMoLogDaoTemp){
	...
	public Integer call(){
		try {
			List<SmsMoLogBean> listSmsMoLogBean = this.gwSmsMoLogDao.queryLtdxByStatus(0);
...
//			}
			for (Iterator<SmsMoLogBean> it = listSmsMoLogBean.iterator(); it.hasNext();) {
				...
				this.gwSmsMoLogDao.updateLtdxStatus(smsMoLogBean);
				smsMoLogBean.setId(PinGen.getSerialPin());
				smsMoLogBean.setStatus(0);
				smsMoLogBean.setType("SMGP");
				smsMoLogBean.setClassify(3);
				this.smsMoLogDao.insert(smsMoLogBean);
				logger.info("LTDX insert end"+ smsMoLogBean.toString());
			}
		} catch(Exception e) {
			logger.error("LtDx mo to up log 短信处理异常",e);
		}
		return 1;
	}
