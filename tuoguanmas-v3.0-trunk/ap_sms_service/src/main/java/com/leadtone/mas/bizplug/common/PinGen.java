...
public class PinGen {
    ...
    public static long getPinPort() {
        return createPinPort(); // 
    }
    
    private static long createPinPort() {
        DecimalFormat numFormat = (DecimalFormat) DecimalFormat.getInstance(); // 格式化数字位数

        Calendar cal = Calendar.getInstance(); // 获取当前时间

        StringBuffer sBuffer = new StringBuffer();
        int year = cal.get(Calendar.YEAR);// 年 1位
        sBuffer.append(Integer.toString(year).substring(3));
...
//    		Thread.sleep(1);
//        }
        System.out.print(PinGen.getBasePin()+"\n");
        System.out.print(PinGen.getCustomerPin()+"\n");
        System.out.print(PinGen.getMerchantPin()+"\n");
        System.out.print(PinGen.getSerialPin()+"\n");
        System.out.print(PinGen.getBatchNo()+"\n");
    }
}
