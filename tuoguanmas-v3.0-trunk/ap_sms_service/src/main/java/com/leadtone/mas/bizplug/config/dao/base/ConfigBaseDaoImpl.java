package com.leadtone.mas.bizplug.config.dao.base;

import java.io.Serializable;
import java.lang.reflect.ParameterizedType;
import java.lang.reflect.Type;
import java.sql.SQLException;
import java.util.Iterator;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.orm.ibatis.SqlMapClientCallback;
import com.ibatis.sqlmap.client.SqlMapClient;
import com.ibatis.sqlmap.client.SqlMapExecutor;
import com.leadtone.mas.bizplug.common.Page;
import com.leadtone.mas.bizplug.common.PageUtil;
import com.leadtone.mas.bizplug.dao.BaseDao;
/*import com.leadtone.mas.bizplug.dao.BaseDao;*/
import com.leadtone.mas.bizplug.sms.bean.MbnSmsHadSend;
import com.leadtone.mas.bizplug.sms.dao.base.SmsBaseIDao;

@SuppressWarnings("unchecked")
public class ConfigBaseDaoImpl<T, PK extends Serializable> extends
		BaseDao implements SmsBaseIDao<T, PK> {

	protected static final String DELETE = ".delete";
	protected static final String INSERT = ".insert";
	protected static final String DELETEBYID=".deleteById";
	protected static final String UPDATE = ".update";
	protected static final String PAGECOUNT = ".pageCount";
	protected static final String PAGE = ".page";
	protected String sqlMapNamespace;
	protected Class<T> type; 
	public void setType(Class<T> type) {
		this.type = type;
		this.sqlMapNamespace = type.getSimpleName();
	}

	/**
	 * 注入数据库操作对象
	 * 
	 * @param sqlMapClient
	 */
	@Autowired
	public void setSqlMapClientAround(
			@Qualifier("sqlMapClient") SqlMapClient sqlMapClient) {
		setSqlMapClient(sqlMapClient);
		Type type = this.getClass().getGenericSuperclass();
		if (type instanceof ParameterizedType) {
			Class<?> clazz = (Class<?>) ((ParameterizedType) type)
					.getActualTypeArguments()[0];
			this.sqlMapNamespace = clazz.getSimpleName();
		}
	}

	public Page page(PageUtil pageUtil) {
		Integer recordes = this.pageCount(pageUtil);
		List<MbnSmsHadSend> results = null;
		if (recordes > 0) {
			results = (List<MbnSmsHadSend>) getSqlMapClientTemplate()
					.queryForList(this.sqlMapNamespace + PAGE, pageUtil);
		}
		return new Page(pageUtil.getPageSize(), pageUtil.getStart(), recordes,
				results);
	}

	public Integer pageCount(PageUtil pageUtil) {
		return (Integer) getSqlMapClientTemplate().queryForObject(
				this.sqlMapNamespace + PAGECOUNT, pageUtil);
	}

	@Override
	public Integer insert(T param) {
		//
		return (Integer) getSqlMapClientTemplate().insert(
				this.sqlMapNamespace + INSERT, param);
	}

	@Override
	public Integer update(T param) {
		//
		return (Integer) getSqlMapClientTemplate().update(
				this.sqlMapNamespace + UPDATE, param);
	}

	@Override
	public Integer delete(T param) {
		//
		return (Integer) getSqlMapClientTemplate().delete(
				this.sqlMapNamespace + DELETE, param);
	}

	@Override
	public T queryByPk(Long pk) {
		//
		return (T) getSqlMapClientTemplate().queryForList(
				this.sqlMapNamespace + ".getByPk", pk);
	}

	@Override
	public T queryByPin(Long pin) {
		//
		return (T) getSqlMapClientTemplate().queryForList(
				this.sqlMapNamespace + ".getByPin", pin);
	}

	/**
	 * 根据参数集合批量修改
	 */
	@Override
	public Integer batchUpdateByList(final List<T> paramList) {
		//
		int executorCount = 1;
		final String sta = this.sqlMapNamespace + UPDATE;
		getSqlMapClientTemplate().execute(new SqlMapClientCallback<T>() {
			public T doInSqlMapClient(SqlMapExecutor executor)
					throws SQLException {
				//
				int executorCount = 0;
				executor.startBatch();
				for (Iterator<T> iterator = paramList.iterator(); iterator
						.hasNext();) {
					T domain = iterator.next();
					executor.update(sta, domain);
					executorCount++;
					if (executorCount % 1000 == 0) {
						executor.executeBatch();
					}
				}
				if (executorCount % 1000 != 0) {
					executor.executeBatch();
				}
				return null;
			}
		});
		return executorCount;
	}

	/**
	 * 根据参数集合批量删除
	 */
	@Override
	public Integer batchDeleteByList(final List<T> paramList) {
		//
		int executorCount = 1;
		final String sta = this.sqlMapNamespace + DELETE;
		getSqlMapClientTemplate().execute(new SqlMapClientCallback<T>() {
			public T doInSqlMapClient(SqlMapExecutor executor)
					throws SQLException {
				//
				int executorCount = 0;
				executor.startBatch();
				for (T pk : paramList) {
					executor.delete(sta, pk);
					executorCount++;
					if (executorCount % 1000 == 0) {
						executor.executeBatch();
					}
				}
				if (executorCount % 1000 != 0) {
					executor.executeBatch();
				}
				return null;
			}
		});
		return executorCount;
	}
	
	/**
	 * 批量保存操作
	 */
	public Integer batchSaveByList(final List<T> entitys){
		// 
        final String sta = this.sqlMapNamespace + INSERT;
        return (Integer) getSqlMapClientTemplate().execute(new SqlMapClientCallback<Integer>() {

            public Integer doInSqlMapClient(SqlMapExecutor executor)
                    throws SQLException {
                int result = 0;
                int executorCount = 0;
                // 
                executor.startBatch();
                for (Iterator<T> iterator = entitys.iterator(); iterator.hasNext();) {
                    T t = iterator.next();
                    executor.insert(sta, t);
                    executorCount++;
                    if (executorCount % 1000 == 0) {
                        result = executor.executeBatch();
                    }
                }
                if (executorCount % 1000 != 0) {
                    result = executor.executeBatch();
                }
                return result;
            }
        });
	}
	/**
	 * 根据ID数组批量删除
	 */
	@Override
	public Integer batchDeleteByPks(final PK[] pks) {
		//
		int executorCount = 1;
		final String sta = this.sqlMapNamespace + DELETEBYID;
		getSqlMapClientTemplate().execute(new SqlMapClientCallback<T>() {
			public T doInSqlMapClient(SqlMapExecutor executor)
					throws SQLException {
				//
				int executorCount = 0;
				executor.startBatch();
				for (Serializable pk : pks) {
					executor.delete(sta, pk);
					executorCount++;
					if (executorCount % 1000 == 0) {
						executor.executeBatch();
					}
				}
				if (executorCount % 1000 != 0) {
					executor.executeBatch();
				}
				return null;
			}
		});
		return executorCount;
	}

	@Override
	public T batchSelectByPks(PK[] pks) {
		//
		return null;
	}
}
