...
public class NOKIAQueryResponse extends NOKIAPacket
{
    ...
    public NOKIAQueryResponse(int sequence_id)
    {
    ...
    public NOKIAQueryResponse(NOKIAPacket packet)
    {
        super(packet);
        query_time = null;
        query_type = 0;
    ...
    public boolean isValid()
    {
        ...
        {
            Log.log("NOKIAQueryResponse.isValid : not a CMPP_QUERY_RESPONSE command !", 0x8600000000000L);
            return false;
        }
        if(query_time == null || query_time.length() != 8)
        {
            Log.log("NOKIAQueryResponse.isValid : invalid query_time !", 0x8600000000000L);
            return false;
        }
        if(query_type != 0 && query_type != 1)
        {
            Log.log("NOKIAQueryResponse.isValid : invalid query_type !", 0x8600000000000L);
            return false;
        }
        if(query_code != null && query_code.length() > 10)
        {
            Log.log("NOKIAQueryResponse.isValid : invalid query_code !", 0x8600000000000L);
            return false;
        } else
        {
            return true;
    ...
    {
        Log.log("NOKIAQueryResponse.wrap : wrap elements !", 0x8800000000000L);
    ...
    public void unwrap()
        throws BufferException
    {
        Log.log("NOKIAQueryResponse.unwrap : unwrap elements !", 0x8800000000000L);
