...
public class EmppReceipterTask implements Callable<Integer> {
	
	private Logger logger = Logger.getLogger(EmppReceipterTask.class);
	
	private ISmsService smsService;
	
	public EmppReceipterTask(){
		if (smsService == null) {
...
//			logger.info("网关短信回执处理开始");
			List<Map<String, Object>> Gatewaylist = smsService.getEmppSmsResult();
			if ((Gatewaylist != null) && (Gatewaylist.size() > 0)) {
				smsService.updateSmsSendRestlt(Gatewaylist);
				smsService.updateEmppSms(Gatewaylist);
			}
